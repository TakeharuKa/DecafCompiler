...
public abstract class Tree {
    ...
    public static final int GUARDED = SEALEDCLASSDEF + 1;
    public static final int IFBRANCH = GUARDED + 1;
    public static final int VAR = IFBRANCH + 1;
    public static final int ARRAYCONSTANT = VAR + 1;
    public static final int ARRAYINIT = ARRAYCONSTANT + 1;
    public static final int ARRAYCONCAT  = ARRAYINIT + 1;
    public static final int ARRAYRANGE = ARRAYCONCAT + 1;
    public static final int ARRAYREF = ARRAYRANGE + 1;
    public static final int ARRAYDEFAULT = ARRAYREF + 1;
    ...
    public static class ClassDef extends Tree {
        ...
        //用于继承
        public ClassDef(int s, Location loc) {
            super(s, loc);
        }
    ...
    public static class Literal extends Expr {
        ...
        public Literal(int typeTag, List<Expr> elist, Location loc) {
            super(LITERAL, loc);
            this.typeTag = typeTag;
            this.elist = elist;
        }
        ...
        public void printTo(IndentPrintWriter pw) {
            switch (typeTag) {
                ...
                case ARRAYCONSTANT:
                    ...
                    pw.incIndent();
                    if (elist.size() == 0) pw.println("<empty>");
                    else {
                        for (Expr e : elist) {
                            e.printTo(pw);
                        }
    ...
    public static class Scopy extends Tree {
        ...
        public Expr value;
        public Scopy(String identname, Expr expr, Location loc) {
            super(SCOPY, loc);
    ...
    public static class Guarded extends Tree {
        public List<Tree> ifBranchList;

        public Guarded(List<Tree> ifBranchList, Location loc) {
            super(GUARDED, loc);
            this.ifBranchList = ifBranchList;
        }
        ...
        public void printTo(IndentPrintWriter pw) {
            pw.println("guarded");
            pw.incIndent();
            if(ifBranchList.isEmpty()) {
                pw.println("<empty>");
            } else {
                for(Tree ifBranch : ifBranchList) {
                    ifBranch.printTo(pw);
                }
            }
            pw.decIndent();
    ...
    public static class IfBranch extends Tree {
        public Expr exp;
        public Tree stmt;
        public IfBranch(Expr exp, Tree stmt, Location loc) {
            super(IFBRANCH, loc);
            this.exp = exp;
            this.stmt = stmt;
        }
    ...
    public static class Var extends Tree {
        ...
        public Var(String ident, Expr expr, Location loc) {
            super(VAR, loc);
            this.ident = ident;
            this.expr = expr;
        }
    ...
    public static class ArrayComp extends Expr {
        public String ident;
        public Expr gexpr, inexpr, boolexpr;

        public ArrayComp(Expr gexpr, String ident, Expr inexpr, Expr boolexpr, Location loc) {
            super(VAR, loc);
            this.ident = ident;
            this.gexpr = gexpr;
            this.inexpr = inexpr;
    ...
    public static class ArrayForeach extends Tree {
        public Tree bv, stmt;
        public Expr whileexpr, inexpr;

        public ArrayForeach(Tree bv, Expr inexpr, Expr whileexpr, Tree stmt, Location loc) {
            super(VAR, loc);
            this.bv = bv;
            this.inexpr = inexpr;
        ...
        public void printTo(IndentPrintWriter pw) {
            pw.println("foreach");
            pw.incIndent();
            if (bv instanceof ArrayVar) {
                pw.println("varbind " + ((ArrayVar)bv).name + " var");
            } else {
                pw.print("varbind " + ((VarDef)bv).name + " ");
                ((VarDef)bv).type.printTo(pw);
                pw.println();
            }
            inexpr.printTo(pw);
            if (whileexpr != null) whileexpr.printTo(pw);
            else pw.println("boolconst true");
            stmt.printTo(pw);
            pw.decIndent();
        }
    }

    public static class ArrayVar extends LValue {
        public String name;

        public ArrayVar(String name, Location loc) {
            super(VAR, loc);
            this.name = name;
        }
    ...
    public static abstract class Visitor {
        ...
        public void visitSealedClassDef(SealedClassDef that) { visitTree(that); }

        public void visitGuarded(Guarded that) { visitTree(that); }

        public void visitIfBranch(IfBranch that) { visitTree(that); }

        public void visitVar(Var that) { visitTree(that); }

        public void visitTernary(Ternary that) { visitTree(that); }

        public void visitArrayComp(ArrayComp that) { visitTree(that); }

        public void visitArrayForeach(ArrayForeach that) { visitTree(that); }

        public void visitArrayVar(ArrayVar that) { visitTree(that); }
